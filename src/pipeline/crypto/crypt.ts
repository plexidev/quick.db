...
export type Cryptor = (
    options: ResolvedCryptOptions,
    data: string,
    encoding?: StringEncoding
) => Promise<string>;

type Payload<K extends string> = Record<K, string>;
type PayloadExtractor<K extends string> = (
    str: string
) => Payload<K> | undefined;
 ...
 */
function makePayloadExtractor<K extends string>(
    template: string
): PayloadExtractor<K> {
...
export class CryptPipeline implements IPipeline<string, string> {
    ...
    constructor(options: CryptOptions) {
        ...
        if (
            !(CryptoAlgorithm as unknown as string[]).includes(
                options.algorithm
            )
        )
            throw new Error("Algorithm is not supported");
        if (!(Encoding as unknown as string[]).includes(options.encoding))
        ...
        this.payloadExtractor =
            makePayloadExtractor<cryptSymbolVars>(cryptSymbol);
        ...
        if (
            ...
            throw new Error(
                "Both an encryptor and decryptor must be provided when using a custom algorithm."
            );
            ...
            this.encryptor ??= async function (options, data): Promise<string> {
                try {
                    ...
                    const stringifiedData =
                        typeof data === "string" ? data : JSON.stringify(data); //(data as unknown) instanceof Object ? JSON.stringify(data) : data;
        ...
        if ("decryptor" in options) {
            ...
            this.decryptor ??= async function (
                ...
                try {
                    ...
                    const iv = binaryData.subarray(
                        binaryData.length - options.byteSize,
                        binaryData.length
                    );
                    const encryptedData = binaryData.subarray(
                        0,
                        binaryData.length - options.byteSize
                    );
    ...
    async deserialize<R>(data: string): Promise<R> {
        try {
            ...
            if (payload.encoding !== "null") {
                if (
                    payload.encoding !== this.options.encoding &&
                    !this.options.solveEncoding
                )
                    throw new Error(
                        `Invalid encoding. Expected '${this.options.encoding}', but got '${payload.encoding}'`
                    );

                if (Encoding.includes(payload.encoding as Encoding))
                    options.encoding = payload.encoding as Encoding;
            }

            const decryptedData = await this.decryptor(
                options,
                payload.payload,
                "utf8"
            );
